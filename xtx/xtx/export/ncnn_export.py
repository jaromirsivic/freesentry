"""NCNN export for XTX-n (spec section 10.2).
...
2. Export to ONNX at fixed input ``(1, 3, 384, 384)``, opset >= 17. We export the
   **raw one-to-one head tensors** (box, cls, kpt) and decode in host code, keeping
   the ONNX/NCNN graph simple and edge-friendly.
3. Simplify with ``onnxsim``.
4. Convert ONNX -> NCNN via ``pnnx`` (preferred) or ``onnx2ncnn``.
5. A raw-NCNN inference path (:class:`NCNNPoseRunner`) loads ``*.param``/``*.bin``,
   runs the 3 crops, decodes, and merges A/B/C in host code.
"""
...
from ..data.preprocess import CROP_KEYS, NETWORK_SIZE, preprocess_image
from ..models import XTXModel
from ..models.head import (
    KPT_CHANNELS_PER_POINT,
    build_anchors,
    decode_boxes,
    decode_keypoints,
    _flatten_level_outputs,
)
...
class ExportModel(nn.Module):
    ...
    def __init__(self, model: XTXModel) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        box, cls, kpt, _ = _flatten_level_outputs(boxes, clss, kpts, self.num_keypoints)
...
def export_onnx(
    ...
    *,
    ...
    # NCNN/pnnx-friendly graph and avoid the onnxscript dependency.
    torch.onnx.export(
        export_model,
        dummy,
        str(output_path),
...
def convert_onnx_to_ncnn(
    ...
    """Convert ONNX -> NCNN ``*.param``/``*.bin`` using pnnx (preferred) or onnx2ncnn."""
    ...
    if converter == "pnnx":
        ...
        subprocess.run([tool, str(onnx_path), f"ncnnparam={param_path}", f"ncnnbin={bin_path}"], check=True)
...
class NCNNPoseRunner:
    """Raw-NCNN inference for the 3-crop XTX-n pipeline (input blob ``in0``)."""

    def __init__(self, param_path: Path, bin_path: Path, *, strides: list[int], num_keypoints: int = 17) -> None:
        try:
    ...
    def _run_single(self, crop_bgr: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ncnn = self._ncnn
        mat = ncnn.Mat.from_pixels(
            np.ascontiguousarray(crop_bgr), ncnn.Mat.PixelType.PIXEL_BGR, crop_bgr.shape[1], crop_bgr.shape[0]
        )
    ...
    def detect(self, image: np.ndarray, *, conf_threshold: float = 0.25, max_detections: int = 300):
        """Run the full A/B/C pipeline and return per-crop raw decodes (for merge in host)."""

        result = preprocess_image(image)
        per_crop = {}
        for crop in CROP_KEYS:
            box, cls, kpt = self._run_single(result.crops[crop])
            ...
            per_crop[crop] = (det, vis, result.transforms[crop])
        return per_crop, result.orig_size


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export XTX-n to NCNN.")
    parser.add_argument("--weights", required=True, help="Trained XTX-n checkpoint")
    parser.add_argument("--out", default="xtx_n", help="Output prefix for ONNX/NCNN files")
