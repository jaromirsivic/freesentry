...
Every A/B/C crop carries a 2x3 affine matrix ``T_k`` mapping
**network-pixel (384 space) -> original-image pixel**. These helpers build,
compose, invert, and apply such matrices to points / boxes / keypoints.

All matrices are ``float64`` numpy arrays of shape ``(2, 3)`` representing::

    [x_out, y_out]^T = M @ [x_in, y_in, 1]^T

The transforms used by XTX are pure scale + translation (axis aligned), but the
helpers accept any 2x3 affine so they remain reusable.
"""
