"""XTX model assembly + builder (spec section 6).

``XTXModel`` wires backbone -> PAN neck -> dual pose head. ``build_model`` constructs
a variant from the scaling table and config. ``fuse`` produces the deployment graph
(Conv+BN folded, RepConv reparameterised, one-to-many head dropped).
"""
...
from .head import HeadOutput, PoseHead, RawPredictions, decode_detections_full
...
class XTXModel(nn.Module):
    """End-to-end XTX pose network (single class, anchor-free, NMS-free)."""
    ...
    def fuse(self) -> "XTXModel":
        ...
        self._fused = True
        logger.info("Fused XTX-%s for deployment (one-to-many head dropped)", self.variant)
        return self
...
def build_model(
    *,
    ...
    """Build an :class:`XTXModel` for ``variant`` honouring ``config['model']``."""
    ...
    return XTXModel(variant=variant, num_keypoints=num_keypoints, use_p2=use_p2)
