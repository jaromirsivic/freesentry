...
class ConvBNAct(nn.Module):
    ...
    def fuse(self) -> "ConvBNAct":
        ...
        if self._fused:
            return self
        fused = _fuse_conv_bn(self.conv, self.bn)
        self.conv = fused
...
class DWConv(nn.Module):
    """Depthwise-separable conv: depthwise KxK + pointwise 1x1 (used by XTX-n)."""
...
class RepConv(nn.Module):
    ...
    At deploy time :meth:`fuse` collapses all branches into a single 3x3 conv with
    bias, matching the RepVGG re-parameterisation.
    """
...
class Bottleneck(nn.Module):
    """Residual bottleneck of two convs; uses RepConv (or DWConv) for the 3x3."""
