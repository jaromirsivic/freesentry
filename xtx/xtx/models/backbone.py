"""XTX backbone (spec section 6.2): 5 stages, strides 2/4/8/16/32.

Exposes feature maps P2 (stride 4), P3 (stride 8), P4 (stride 16), P5 (stride 32).
For a 384 input these are 96/48/24/12 spatial sizes. ``XTX-n`` uses depthwise
separable downsampling/blocks; ``m``/``l`` use full CSP blocks. The deepest stage
ends with SPPF for a large receptive field.
"""
...
from .blocks import ConvBNAct, CSPBlock, DWConv, SPPF
from .scaling import VariantConfig, scaled_channels, scaled_depths


def _downsample(*, in_channels: int, out_channels: int, depthwise: bool) -> nn.Module:
    if depthwise:
...
class Backbone(nn.Module):
    """CSPDarknet-style backbone producing {P2, P3, P4, P5}."""
    ...
    def __init__(self, *, cfg: VariantConfig) -> None:
        ...
        # Stage 1 -> P2 (stride 4)
        self.down1 = _downsample(in_channels=channels[0], out_channels=channels[1], depthwise=dw)
        self.stage1 = CSPBlock(
            in_channels=channels[1], out_channels=channels[1], num_blocks=depths[0], depthwise=dw
        )

        # Stage 2 -> P3 (stride 8)
        self.down2 = _downsample(in_channels=channels[1], out_channels=channels[2], depthwise=dw)
