...
``annotations`` across files, groups annotations by ``image_id`` and resolves image
paths relative to each JSON's directory. Bounding boxes and keypoints are stored
...
class PersonAnnotation:
    ...
    keypoints_norm: np.ndarray  # (K, 3) float32, [x_norm, y_norm, vis]
    iscrowd: int = 0
...
def _parse_single_json(
    json_path: Path, *, num_keypoints: int
) -> dict[Path, ImageSample]:
    try:
    ...
    for ann in annotations_raw:
        ...
        if bbox is None or keypoints is None:
            logger.warning(
                "Skipping malformed annotation (image_id=%s) in %s", image_id, json_path
            )
            continue
...
def scan_split(split_dir: Path, *, num_keypoints: int = NUM_KEYPOINTS) -> list[ImageSample]:
    ...
    Images appearing in multiple JSON files are merged (annotations concatenated).
    """
    ...
    samples = [s for s in merged.values() if s.annotations or True]
    logger.info(
...
def dataset_statistics(samples: list[ImageSample], *, num_keypoints: int = NUM_KEYPOINTS) -> dict:
    ...
    for sample in samples:
        for ann in sample.annotations:
            num_people += 1
            vis = ann.keypoints_norm[:, 2].astype(np.int64)
