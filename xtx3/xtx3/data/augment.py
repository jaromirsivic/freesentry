"""Geometric + photometric augmentation for the nine-landmark task.

Augmentation is applied at the **placement stage** so geometry stays consistent
across views:

* Horizontal flip (with left/right swap of eyes, ears, shoulders, and hips) and
  photometric jitter operate on the original image / pixel-space targets before
  letterboxing.
* Random scale jitter and translation perturb the **letterbox placement**
  itself (``CanvasPlacement`` scale/offset). In pyramid mode this varies which
  people land in the centre views; in whole-frame mode it varies person scale
  and framing -- which also teaches the ``u`` variant to tolerate the
  256/320/384 resolution range.
...
Keypoints that leave the transformed frame are never marked visible: the
dataset target transform zeroes the visibility of out-of-frame points.
"""
...
from ..utils.keypoints import NUM_KEYPOINTS, build_flip_index
...
def jittered_placement(
    *,
    ...
    """Random scale + translation of the letterbox placement.

    Scale multiplies the canonical letterbox scale by ``U(1-j, 1+j)`` and the
    paste offset is shifted by up to ``translate * canvas_size`` in each axis.
    """
...
class Augmentor:
    ...
    def placement(self, *, width: int, height: int, canvas_size: int) -> CanvasPlacement:
        """Return the (possibly jittered) letterbox placement for this sample."""

        if not self.enabled:
            return compute_placement(width=width, height=height, canvas_size=canvas_size)
...
def mosaic4(
    ...
    *,
    ...
    kpts_arr = (
        np.concatenate(out_kpts, axis=0)
        if out_kpts
