"""Total XTX3 loss: box + cls + keypoint + visibility, with **ProgLoss
(Progressive Loss Balancing)** across the dual head.
...
via :meth:`XTX3Loss.set_progress`; the current weights are exposed for the
per-step log.
...
visibility cross-entropy over the nine landmarks. Points with GT visibility 0
are masked out of the localisation loss.

Multi-scale training (XTX3-u): ``forward`` accepts the actual ``image_size`` of
the current batch so the centre prior scales with the input resolution.
"""
...
class XTX3Loss(nn.Module):
    """Weighted multi-task loss for the XTX3 dual head with ProgLoss balancing."""
    ...
    @classmethod
    def from_config(cls, config: dict[str, Any]) -> "XTX3Loss":
    ...
    def forward(
        ...
        *,
        ...
        size = int(image_size) if image_size is not None else self.network_size
        w_o2m, w_o2o = self.current_weights()

        o2o_losses = self._head_loss(output.one2one, targets, self.assigner_o2o, size)
        if output.one2many is not None:
            o2m_losses = self._head_loss(output.one2many, targets, self.assigner_o2m, size)
        else:
