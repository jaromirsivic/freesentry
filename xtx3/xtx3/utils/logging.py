"""Centralised logging configuration for the XTX3 package.
...
def configure_logging(*, level: int = logging.INFO) -> None:
    """Install a single stream handler on the root ``xtx3`` logger.
...
def add_file_handler(log_path: Path | str, *, level: int = logging.INFO) -> None:
    """Mirror all ``xtx3`` log output into ``log_path`` (appending).
    ...
    logging.getLogger("xtx3").addHandler(handler)
