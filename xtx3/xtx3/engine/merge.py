"""Cross-view merge (dedup) + calibration.

Used only in **pyramid mode**: after decoding detections from views 1, 2, 3 and
mapping them to original-image coordinates, the same person seen in multiple
views must be deduplicated with deterministic priority **view 1 > view 2 >
view 3** (keep the full-frame detection, add view 2 for people it missed, add
view 3 for the farthest central people view 2 also missed). Because XTX3 is
NMS-free within a view, only cross-view duplicates are resolved here. A
centre-distance spatial short-circuit keeps the pairwise pass O(n) in practice.
Whole-frame mode has a single view (key 0) so the merge degenerates to a
confidence filter.

The matching thresholds are calibrated on the test split (grid search to
maximise merged OKS mAP), with an optional logistic "same person" classifier
(``merge.mode: "threshold" | "logistic"``). Calibrated params are persisted in
the checkpoint and mirrored into ``config.json``.
"""
...
from ..utils.keypoints import NUM_KEYPOINTS, OKS_SIGMAS
from ..utils.logging import get_logger
from .metrics import ImageGroundTruth, ImagePredictions, evaluate

logger = get_logger(__name__)

# Deterministic dedup priority: whole-frame view first (trivially unique), then
# pyramid views coarse-to-fine.
VIEW_PRIORITY = (0, 1, 2, 3)
...
class Detection:
    ...
    source_view: int  # 0 (whole) | 1 | 2 | 3 (pyramid)


@dataclass(slots=True)
class MergeParams:
    """Calibrated cross-view merge parameters (persisted in checkpoint/config)."""
    ...
    # [w_iou, w_oks, w_center_dist, w_scale_ratio, w_view_pair, bias]
    logistic_weights: list[float] = field(default_factory=list)
...
def _oks_between(det_a: Detection, det_b: Detection, sigmas: np.ndarray) -> float:
    """OKS between two detections using the mean of the two box areas."""
...
def pair_features(det_a: Detection, det_b: Detection, sigmas: np.ndarray) -> np.ndarray:
    """Pairwise features ``[IoU, OKS, center_distance, scale_ratio, view_pair]``."""
    ...
    view_pair = abs(det_a.source_view - det_b.source_view) / 2.0
    return np.array([iou, oks, center_dist, scale_ratio, view_pair], dtype=np.float64)
...
def _is_match(det_a: Detection, det_b: Detection, params: MergeParams, sigmas: np.ndarray) -> bool:
    ...
    if params.mode == "logistic" and len(params.logistic_weights) == 6:
        feats = pair_features(det_a, det_b, sigmas)
        weights = np.array(params.logistic_weights[:5])
        bias = params.logistic_weights[5]
...
def merge_views(
    by_view: dict[int, list[Detection]],
    *,
    params: MergeParams,
    sigmas: np.ndarray | None = None,
) -> list[Detection]:
    """Merge per-view detections with priority view 1 > view 2 > view 3.

    Whole-frame inference passes a single view (key 0), reducing this to a
    ...
    for view in VIEW_PRIORITY:
        candidates = [d for d in by_view.get(view, []) if d.score >= params.conf_threshold]
...
def _detections_to_predictions(dets: list[Detection]) -> ImagePredictions:
    ...
    vis = np.stack([d.visibility for d in dets], axis=0)
    return ImagePredictions(keypoints=kpts, scores=scores, visibility=vis)


def calibrate_thresholds(
    per_image_views: list[dict[int, list[Detection]]],
    ...
    for conf in conf_grid:
        for oks_thr in oks_grid:
            for iou_thr in iou_grid:
                ...
                preds = [
                    _detections_to_predictions(merge_views(views, params=trial, sigmas=sigmas))
                    for views in per_image_views
                ]
...
def fit_logistic(
    ...
    *,
    ...
    Feature columns: ``[IoU, OKS, center_distance, scale_ratio, view_pair]``;
    returns ``[w_iou, w_oks, w_cd, w_sr, w_vp, bias]`` for ``merge.mode="logistic"``.
    """
