"""Checkpoint management + EMA.

Checkpoints store model/EMA/optimizer/scaler state, epoch/step, the ProgLoss
schedule position (via ``global_step`` + criterion state), a frozen config
snapshot, merge params (filled after calibration), metrics, and the nine-point
keypoint metadata. EMA weights are used for validation and export.
"""
...
def discover_checkpoints(runs_dir: Path) -> list[Path]:
    """Find resumable checkpoints (``runs/**/last.pt``, ``runs/**/best.pt``)."""

    runs_dir = Path(runs_dir)
    if not runs_dir.is_dir():
        return []
    found = sorted(runs_dir.glob("**/last.pt")) + sorted(runs_dir.glob("**/best.pt"))
