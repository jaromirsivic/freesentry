"""Decoupled, dual, NMS-free pose head with RLE keypoints (nine landmarks).

Per pyramid level a decoupled head emits three branches:

* **box**: 4 direct distance-to-edge values ``(l, t, r, b)`` -- no DFL --
  decoded with the level stride and a **learnable per-level scale**.
...
* **keypoint (RLE)**: per keypoint ``(dx, dy, log_sigma, vis0, vis1, vis2)`` where
  ...
  class. Per-keypoint confidence = P(occluded) + P(visible).

With nine keypoints the keypoint branch emits 54 channels per location (versus
102 in the 17-point XTX2 head), which is the main structural saving of XTX3.

Two structurally identical heads are trained jointly (YOLO26 style):
...
``drop_one2many()`` removes the auxiliary head for deployment. The head is
fully convolutional: anchors are rebuilt from the actual feature-map sizes on
every forward, so any input size divisible by 32 works (256/320/384 for ``u``).
"""
...
class RawPredictions:
    ...
    Shapes (``N`` = total anchors across levels):
        box:     (B, N, 4)        raw ltrb (pre-activation, already scale-adjusted)
        cls:     (B, N, 1)        logits
        kpt:     (B, N, K, 6)     raw (dx, dy, log_sigma, vis0, vis1, vis2)
        anchors: (N, 2)           anchor centre in network pixels
...
class SingleHead(nn.Module):
    """One decoupled head (box + cls + kpt) with a learnable per-level box scale."""
    ...
    def __init__(
        ...
        *,
        ...
        # up the parameter/FLOP budget on wide deep stages. Caps come from the
        # variant config -- the u variant uses the tightest caps because the head
        # dominates its FLOPs.
        ...
        # Learnable per-level scale on the ltrb regression.
        self.box_scales = nn.Parameter(torch.ones(self.num_levels))
    ...
    def forward(
        ...
        for i, feat in enumerate(features):
            boxes.append(self.box_branches[i](feat) * self.box_scales[i])
...
def decode_boxes(raw_box: torch.Tensor, anchors: torch.Tensor, strides: torch.Tensor) -> torch.Tensor:
    ...
    positive with softplus and scaled by the level stride (DFL-free direct
    regression).
    """
...
def decode_keypoints(
    ...
    the probability the point is present (occluded or visible).
    """
...
class PoseHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.one2one = SingleHead(
            in_channels_list=in_channels_list, num_keypoints=num_keypoints,
            depthwise=depthwise, box_cap=box_cap, kpt_cap=kpt_cap,
        )
    ...
    def _assemble(self, head: SingleHead, features: list[torch.Tensor]) -> RawPredictions:
        boxes, clss, kpts = head(features)
        box, cls, kpt, sizes = flatten_level_outputs(boxes, clss, kpts, self.num_keypoints)
...
def decode_detections_full(
    ...
    *,
    ...
    """Decode one head's raw predictions into per-image detections.

    Returns ``(dets, vis)`` where ``dets[i]`` is ``(num_det, 32)``
    (``[bbox xyxy(4), score(1), 9*(x, y, conf)]`` in network pixels) and
    ``vis[i]`` is ``(num_det, K)`` int64 visibility classes for image ``i``.
    No NMS anywhere: the one-to-one head is trained to emit unique detections.
    """
...
def decode_detections(
    ...
    *,
    ...
    """Like :func:`decode_detections_full` but detections only."""
