"""XTX2 Pose - interactive console training entry point (spec section 9).

Usage:
    python train.py                              # interactive prompts
    python train.py --variant n --dataset ./dataset
    python train.py --resume runs/xtx2-n/last.pt
"""
...
from xtx2.engine.checkpoint import load_checkpoint
from xtx2.engine.trainer import (
    Trainer,
    load_config,
    prompt_dataset,
    prompt_output,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an XTX2 pose model (n/m/l).")
...
def main() -> None:
    ...
    # Resume question first (section 9.1); silently skipped when no checkpoint
    # exists. On resume, variant and config come from the checkpoint and the
    ...
    if resume_path is not None:
        ...
        logger.info("Resuming XTX2-%s from %s", variant, resume_path)
    else:
