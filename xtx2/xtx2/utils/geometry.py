...
Every pyramid level carries a 2x3 float64 affine matrix ``T_k`` mapping
**network-pixel (384 space) -> original-image pixel** (spec section 4.4). These
helpers build, compose, invert, and apply such matrices to points / boxes /
keypoints so detections map back with one matrix multiply.

All matrices are ``float64`` numpy arrays of shape ``(2, 3)`` representing::

    [x_out, y_out]^T = M @ [x_in, y_in, 1]^T

XTX2 transforms are pure scale + translation (no rotation), but the helpers
