...
* **keypoint (RLE)**: per keypoint ``(dx, dy, log_sigma, vis0, vis1, vis2)`` where
  ...
  class (section 7.2). Per-keypoint confidence = P(occluded) + P(visible).
...
class SingleHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # The depthwise (n) variant uses tighter caps -- the head dominates its
        # FLOPs and this is one of the "faster than XTX" levers.
        box_cap = 64 if depthwise else 80
        kpt_cap = 96 if depthwise else 128
        ...
        # Learnable per-level scale on the ltrb regression (section 6.4 box branch).
...
def decode_keypoints(
    ...
    the probability the point is present (occluded or visible), section 7.2.
    """
...
def decode_detections_full(
    ...
    *,
    ...
    Returns ``(dets, vis)`` where ``dets[i]`` is ``(num_det, 56)`` (section 6.5:
    ``[bbox xyxy(4), score(1), 17*(x, y, conf)]`` in 384-network pixels) and
