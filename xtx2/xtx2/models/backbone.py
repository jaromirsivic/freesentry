"""XTX2 backbone (spec section 6.2): 5 stages, strides 2/4/8/16/32.

Exposes feature maps P2 (stride 4), P3 (stride 8), P4 (stride 16), P5 (stride 32).
For a 384 input these are 96/48/24/12 spatial sizes. ``XTX2-n`` uses a cheap
**PConv stem** and PConv/depthwise-separable early stages (Raspberry Pi FLOPs
budget, section 6.1); ``m``/``l`` use full CSP blocks. The deepest stage ends
with SPPF for a large receptive field.
"""
...
def _downsample(*, in_channels: int, out_channels: int, depthwise: bool, early: bool = False) -> nn.Module:
    if depthwise and early:
        # PConv downsampling for the n variant's high-resolution early stages.
...
class Backbone(nn.Module):
    ...
    def __init__(self, *, cfg: VariantConfig) -> None:
        ...
        # Stem: stride 2. The n variant uses a plain 3x3 stem conv followed by
        ...
        # Stage 1 -> P2 (stride 4). Early stage: PConv path for n.
        ...
        # Stage 2 -> P3 (stride 8). Early stage: PConv path for n.
