"""Re-usable network building blocks (spec section 6.1).
...
New relative to XTX: :class:`PConv` (partial convolution, FasterNet-style) --
convolve only a fraction of channels and pass the rest through -- used in the
``n`` stem and early stages as one of the concrete "faster than XTX" levers.
"""
...
class DWConv(nn.Module):
    """Depthwise-separable conv: depthwise KxK + pointwise 1x1 (used by XTX2-n)."""
...
class PConv(nn.Module):
    """Partial convolution (FasterNet-style, spec section 6.1).
    ...
    early, high-resolution stages -- one of the "faster than XTX" levers for the
    ``n`` variant.
    """
