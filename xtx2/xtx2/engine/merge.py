"""Cross-level merge (dedup) + calibration (spec section 5).

After decoding detections from levels 1, 2, 3 and mapping them to original-image
coordinates, the same person seen in multiple levels must be deduplicated with
priority **level_1 > level_2 > level_3**: keep level_1's detection, add level_2
for people it missed, add level_3 for the farthest central people level_2 also
missed. Because XTX2 is NMS-free within a level, only cross-level duplicates are
resolved here. A centre-distance spatial short-circuit keeps the pairwise pass
O(n) in practice.
...
class Detection:
    ...
    source_level: int  # 1 | 2 | 3


@dataclass(slots=True)
class MergeParams:
    """Calibrated cross-level merge parameters (persisted in checkpoint/config)."""
    ...
    # [w_iou, w_oks, w_center_dist, w_scale_ratio, w_level_pair, bias]
...
def _oks_between(det_a: Detection, det_b: Detection, sigmas: np.ndarray) -> float:
    """OKS between two detections using the mean of the two box areas (section 5.1)."""
...
def pair_features(det_a: Detection, det_b: Detection, sigmas: np.ndarray) -> np.ndarray:
    """Pairwise features ``[IoU, OKS, center_distance, scale_ratio, level_pair]``."""
    ...
    level_pair = abs(det_a.source_level - det_b.source_level) / 2.0
    return np.array([iou, oks, center_dist, scale_ratio, level_pair], dtype=np.float64)


def _is_match(det_a: Detection, det_b: Detection, params: MergeParams, sigmas: np.ndarray) -> bool:
    # Spatial short-circuit (section 5.2): skip pairs whose centres are far apart.
...
def merge_levels(
    by_level: dict[int, list[Detection]],
    *,
    params: MergeParams,
    sigmas: np.ndarray | None = None,
) -> list[Detection]:
    """Merge per-level detections with priority level_1 > level_2 > level_3."""

    sigmas = OKS_SIGMAS if sigmas is None else sigmas
    accepted: list[Detection] = []

    for level in LEVEL_PRIORITY:
        candidates = [d for d in by_level.get(level, []) if d.score >= params.conf_threshold]
...
def calibrate_thresholds(
    per_image_levels: list[dict[int, list[Detection]]],
    ...
    *,
    ...
    """Grid-search ``(oks_thr, iou_thr, conf)`` to maximise merged OKS AP (5.3)."""
    ...
    for conf in conf_grid:
        for oks_thr in oks_grid:
            for iou_thr in iou_grid:
                ...
                preds = [
                    _detections_to_predictions(merge_levels(levels, params=trial, sigmas=sigmas))
                    for levels in per_image_levels
                ]
...
def fit_logistic(
    ...
    *,
    ...
    Feature columns: ``[IoU, OKS, center_distance, scale_ratio, level_pair]``;
    returns ``[w_iou, w_oks, w_cd, w_sr, w_lp, bias]`` for ``merge.mode="logistic"``.
    """
