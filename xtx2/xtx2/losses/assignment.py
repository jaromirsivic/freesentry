...
* **STAL (Small-Target-Aware Label Assignment)** -- adopted from YOLO26: for GT
  boxes smaller than ``stal_size_px`` (default 8 px in 384 network space, i.e.
  smaller than one cell at the finest stride), the geometry used for candidate
  *selection* is decoupled from the geometry used for *regression*: the
  assignment surrogate box is inflated so the GT is guaranteed a minimum of
  ``stal_min_anchors`` (default 4) positive candidates, while regression still
  targets the true box. Critical for level_3's distant people and tiny people
  in level_1.
* **Centre prior (XTX2-specific)**: anchors near the image centre receive a
  bonus in the alignment metric, aligning the network with the pyramid's
  centre-recovery objective.
"""
