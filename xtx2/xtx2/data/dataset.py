"""PyTorch dataset producing explicit level_1/2/3 training samples (spec 8.3-8.4).

Each source image yields three samples (level 1, 2, 3); a :class:`BalancedSampler`
controls the level ratio. The target transform maps normalised-original
annotations into 384 network space via ``invert(T_k)`` -- the exact same geometry
as inference -- and applies the inclusion / visibility rules (people outside a
level's field of view produce no target there, which teaches scale
specialisation).
"""
...
def targets_to_network(
    ...
    *,
    ...
    """Map original-pixel targets into a level's 384 network space (section 8.3).

    Returns ``(boxes (M',4), keypoints (M',K,3))`` keeping only people whose bbox
    centre falls inside the level's frame and that retain at least
...
def level_to_tensor(level_bgr: np.ndarray) -> torch.Tensor:
    """Convert a uint8 ``(384,384,3)`` BGR level to a float ``(3,384,384)`` tensor /255."""

    return torch.from_numpy(np.ascontiguousarray(level_bgr)).permute(2, 0, 1).float() / 255.0
...
class SampleMeta:
    image_path: str
    level: int
    transform: np.ndarray  # T_k, network (384) -> source-image px
    orig_size: tuple[int, int]


class XTX2PoseDataset(Dataset):
    """Dataset of level_1/2/3 samples with network-space targets."""
    ...
    def __init__(
        ...
        *,
        ...
        # Flattened (sample_idx, level) index: three entries per source image.
        self.index: list[tuple[int, int]] = [
            (i, level) for i in range(len(self.samples)) for level in LEVEL_KEYS
        ]
    ...
    def _load(self, sample_idx: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if image is None:
            ...
            image = np.zeros((sample.height or NETWORK_SIZE, sample.width or NETWORK_SIZE, 3), np.uint8)
    ...
    def __getitem__(self, i: int) -> tuple[torch.Tensor, dict[str, torch.Tensor], int, SampleMeta]:
        ...
        height, width = image.shape[:2]
        placement = self.augmentor.placement(width=width, height=height)
        transforms = transforms_from_placement(placement)
        transform = transforms[level]  # network (384) -> (augmented) source px

        # Only the requested level's pixels are rendered (one-shot, section 4.3).
        from .preprocess import _render_level  # noqa: PLC0415 - avoid a public alias

        level_bgr = _render_level(image, placement, level)
        ...
        tensor = level_to_tensor(level_bgr)
        targets = {
        ...
        return tensor, targets, level, meta

    def per_level_target_counts(self) -> dict[int, int]:
        """Count valid targets per level over the whole split (dataset stats)."""

        counts = {level: 0 for level in LEVEL_KEYS}
        for sample in self.samples:
            width = sample.width or 1
            height = sample.height or 1
            boxes, kpts = denormalise_sample(sample, width=width, height=height)
            from .preprocess import compute_transforms  # noqa: PLC0415
            ...
            for level in LEVEL_KEYS:
                ...
                counts[level] += net_boxes.shape[0]
        return counts
...
def collate_fn(
    ...
    levels = [item[2] for item in batch]
    metas = [item[3] for item in batch]
    return images, targets, levels, metas


class BalancedSampler(Sampler[int]):
    """Samples flattened indices to honour the configured level_1:2:3 ratio."""

    def __init__(
        self,
        *,
        index: list[tuple[int, int]],
        level_ratio: tuple[float, float, float] = (1.0, 1.0, 1.0),
        shuffle: bool = True,
        seed: int = 0,
    ) -> None:
        self.index = index
        ...
        self.seed = seed
        self.epoch = 0
        ratio_map = {1: level_ratio[0], 2: level_ratio[1], 3: level_ratio[2]}
        total = sum(ratio_map.values()) or 1.0
        self.weights = torch.tensor(
            [ratio_map[level] / total for (_, level) in index], dtype=torch.double
        )
